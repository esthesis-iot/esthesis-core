...
public class ControlCommandHealth {
  ...
  private static final String COMMAND_NAME = CommandType.HEALTH;
  ...
  public void receiveCommand(CommandRequestDTO cmd) {
    ...
    // Check if hardware data are available.
    if (HealthMetadataCollector.hwPlatformError) {
      log.log(Level.WARNING, "Could not process ''HEALTH'' command as hardware information could not be collected.");
      return;
    }
