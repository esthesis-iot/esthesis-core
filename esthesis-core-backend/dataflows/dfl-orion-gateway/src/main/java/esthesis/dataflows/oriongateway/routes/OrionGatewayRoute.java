...
/**
 * Camel routes to handle messages from Kafka and send them to Orion.
 */
@Slf4j
@ApplicationScoped
public class OrionGatewayRoute extends RouteBuilder {
    ...
    if (config.kafkaApplicationTopic().isPresent()) {
      log.info("Listening for application messages on Kafka topic '{}'.",
          config.kafkaApplicationTopic().get()); //NOSONAR

      from("kafka:" + config.kafkaApplicationTopic().get()) //NOSONAR
    ...
    if (config.kafkaTelemetryTopic().isPresent()) {
      log.info("Creating route from Kafka topic '{}'.", config.kafkaTelemetryTopic().get()); //NOSONAR

      from("kafka:" + config.kafkaTelemetryTopic().get()) //NOSONAR
    ...
    if (config.kafkaMetadataTopic().isPresent()) {
      log.info("Creating route from Kafka topic '{}'.", config.kafkaMetadataTopic().get()); //NOSONAR

      from("kafka:" + config.kafkaMetadataTopic().get()) //NOSONAR
				.unmarshal(EsthesisDataMessageDataFormat.create())
				.to("seda:metadata");
      from("seda:metadata")
          .bean(orionGatewayService, "processData");
