...
public class OrionKeyrockAuthService implements OrionAuthService {
	...
	private String getValidToken() {
		if (Objects.isNull(cachedAccessToken) || Instant.now().isAfter(cachedAccessToken.getExpirationTime())) {
			renewToken();
		}
	...
	private void renewToken() {
		...
		cachedAccessToken = keyrockAuthClient.requestAccessToken(username, password, grantType, basicToken);
		...
		cachedAccessToken.setExpirationTime(Instant.now().plus(expirationSecondsAdjusted, ChronoUnit.SECONDS));
