...
import esthesis.services.dashboard.impl.dto.update.DashboardUpdateDiff;
...
/**
 * Helper class for updating the DIFF dashboard item.
 */
@Slf4j
@ApplicationScoped
public class DiffUpdateJobHelper extends UpdateJobHelper<DashboardUpdateDiff> {

	@Inject
	RedisUtils redisUtils;

	public DashboardUpdateDiff refresh(DashboardEntity dashboardEntity, DashboardItemDTO item) {
		DashboardUpdateDiff.DashboardUpdateDiffBuilder<?, ?> replyBuilder =  DashboardUpdateDiff.builder()
			.id(item.getId()).type(Type.SENSOR);

		try {
			// Get item configuration & security checks.
			DashboardItemSensorConfiguration config = getConfig(DashboardItemSensorConfiguration.class, item);
			...
			log.error("Error processing '{}' for dashboard item '{}'.", Type.DIFF, item.getId(), e);
