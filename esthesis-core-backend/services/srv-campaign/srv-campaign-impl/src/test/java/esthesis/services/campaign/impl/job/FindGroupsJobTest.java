...
@QuarkusTest
class FindGroupsJobTest {

	@Inject
	FindGroupsJob findGroupsJob;
	...
	void handle() {
		...
		assertDoesNotThrow(() -> findGroupsJob.handle(jobClient, activatedJob));

		// Assert that the groups were found.
		assertFalse(parameters.getGroups().isEmpty());
	}
}
