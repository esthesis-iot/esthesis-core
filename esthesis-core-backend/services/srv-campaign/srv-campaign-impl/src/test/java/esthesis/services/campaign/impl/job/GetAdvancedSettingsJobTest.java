...
@QuarkusTest
class GetAdvancedSettingsJobTest {

	@Inject
	GetAdvancedSettingsJob getAdvancedSettingsJob;
	...
	void handle() {
		...
		// Assert that the job can be handled without exceptions and the settings are set.
		assertDoesNotThrow(() -> getAdvancedSettingsJob.handle(jobClient, activatedJob));
		assertNotNull(parameters.getAdvancedDateTimeRecheckTimer());
		assertNotNull(parameters.getAdvancedPropertyRecheckTimer());
		assertNotNull(parameters.getAdvancedUpdateRepliesTimer());
		assertNotNull(parameters.getAdvancedUpdateRepliesFinalTimer());
