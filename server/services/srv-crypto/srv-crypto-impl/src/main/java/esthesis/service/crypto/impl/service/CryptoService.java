...
public class CryptoService extends CryptoConverters {
  ...
  throws OperatorCreationException, CertIOException {
    ...
    certGenerator = new X509v3CertificateBuilder(new X500Name(
        CN + "=" + StringUtils.defaultIfBlank(
            certificateSignRequestDTO.getIssuerCN(),
    ...
    certHolder = certGenerator.build(
        new JcaContentSignerBuilder(
            certificateSignRequestDTO.getSignatureAlgorithm()).build(
  ...
  throws NoSuchAlgorithmException, NoSuchProviderException {
    ...
    // Set the provider.
    if (StringUtils
        .isNotBlank(createKeyPairRequestDTO.getKeyPairGeneratorAlgorithm()) &&
        StringUtils
            .isNotBlank(createKeyPairRequestDTO.getKeyPairGeneratorProvider())) {
      keyPairGenerator =
          KeyPairGenerator
              .getInstance(createKeyPairRequestDTO.getKeyPairGeneratorAlgorithm(),
                  createKeyPairRequestDTO.getKeyPairGeneratorProvider());
    } else {
      keyPairGenerator =
          KeyPairGenerator.getInstance(createKeyPairRequestDTO.getKeyPairGeneratorAlgorithm());
    }
    ...
    keyPairGenerator.initialize(createKeyPairRequestDTO.getKeySize(),
        SecureRandom.getInstance(getSecureRandomAlgorithm(
            createKeyPairRequestDTO.getSecureRandomAlgorithm())));
   ...
   */
  public byte[] saveCertificateToKeystore(final byte[] keystore,
  ...
  throws NoSuchAlgorithmException, CertificateException, NoSuchProviderException, KeyStoreException,
         IOException {
    // Load the keystore.
    KeyStore ks = keystoreDeserialize(keystore, keystoreType,
  ...
  public byte[] savePrivateKeyToKeystore(final byte[] keystore, final String keystoreType,
      ...
      final PrivateKey key, final String keyPassword, final Set<byte[]> certificates)
  throws NoSuchAlgorithmException, CertificateException, NoSuchProviderException, KeyStoreException,
         IOException {
    // Load the keystore.
    KeyStore ks = keystoreDeserialize(keystore, keystoreType,
