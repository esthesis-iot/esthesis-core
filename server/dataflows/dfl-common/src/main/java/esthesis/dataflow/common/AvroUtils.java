...
public class AvroUtils {
  ...
  public PayloadData parsePayload(final String line) {
    ...
    PayloadData.Builder payloadBuilder = EsthesisDataMessage.newBuilder()
        .getPayloadBuilder();
   ...
   */
  public EsthesisCommandReplyMessage parseCommandReplyLP(String body,
      String hardwareId, String appName, String topic) {
    // Parse the command reply message.
    try {
      String correlationId = body.substring(0, body.indexOf(
          " "));
      String success = body.substring(body.indexOf(" ") + 1,
          body.indexOf(" ") + 2);
      String payload = body.substring(body.indexOf(" ") + 3);

      log.debug(
          "Extracted correlation ID '{}', success '{}', and payload '{}'.",
          correlationId, success, abbr(payload));
      ...
      log.debug(
          "Parsed Command Reply message to EsthesisCommandReplyMessage '{}'",
          abbr(msg.toString()));

      return msg;
    } catch (Exception e) {
      throw new QMismatchException(
          "Failed to parse Command Reply message '{}' due to '{}'. "
              + "Check that Command Reply message are formatted as [correlationId] "
              + "[success] [output].",
          DflUtils.abbr(body), e);
    }
  }
  ...
  public String commandRequestToLineProtocol(
    ...
    log.debug("Converted Avro Command Request message '{}' to line protocol "
        + "message '{}'.", abbr(msg.toString()), abbr(lineProtocol.toString()));
